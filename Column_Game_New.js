        ...
        box[i][j].element.addEventListener('click', function() {
            if (turn == 1) {
                if(colClick != 0 && j + 1 != colClick && numClicked > 0) {
                    ...
                    if(box[i][j].state == 0) {box[i][j].state = 1; numClicked++; box[i][j].x[0].style = "display: inline-block; stroke:#666; stroke-width:2;"; box[i][j].x[1].style = "display: inline-block; stroke:#666; stroke-width:2;";}
                    else if(box[i][j].state == 1) {box[i][j].state = 0; numClicked--; box[i][j].x[0].style = "display: none;"; box[i][j].x[1].style = "display: none;";}
...
confirmMoveButton.addEventListener('click', function() {
    ...
    if (turn == 0) {
        ...
        if (numClicked == 0) {setMessage ('You must click at least one box!', 'red'); return;}
        if (numClicked > (winTotal - totalClicked)) {setMessage ('You are not allowed to click all the boxes!', 'red'); return;};
        totalClicked += numClicked;
        rowsStruck[colClick - 1] += numClicked;
        let counter = 0;
        for (let i = 0; i < colClick; i++) {
            if(box[i][colClick - 1].state != 2 && counter < numClicked) {box[i][colClick - 1].state = 2; box[i][colClick - 1].x[0].style = "display: inline-block; stroke:#111; stroke-width:3;"; box[i][colClick - 1].x[1].style = "display: inline-block; stroke:#111; stroke-width:3;"; counter++;}
        ...
        if (totalClicked == winTotal) {gameWon(turn); gameOver = 1; return;}
...
confirmMoveButton.value = "Confirm Computer's Move";
    ...
    if (oppType == 0) {
        ...
        if (win == 1) {
            ...
            while (winValue[compIndex] != 1 && counter < 100);
        }
    }
}
...
function confirmComputerMove() {
    ...
    for (let i = 0; i < colClick; i++) {
        if(box[i][colClick - 1].state != 2 && counter < numClicked) {
            ...
            box[i][colClick - 1].x[1].style = "display: inline-block; stroke:#666; stroke-width:2;"; counter++;
        }
    };
}
...
// Listen for the user to restart the game.
...
function gameWon() {
    ...
    for (i = 0; i < columns; i++) {
        if (rowsStruck[i] < i + 1) {
            {box[i][i].state = 2; 
             box[i][i].x[0].style = "display: inline-block; stroke: darkred; stroke-width:2;"; 
             box[i][i].x[1].style = "display: inline-block; stroke: darkred; stroke-width:2;";}
        }
    }
}
